    ...
    return (
        <nav className="flex items-center justify-between flex-wrap bg-teal-500 p-6">
            ...
            <div className="block lg:hidden">
                ...
                <div className="absolute border right-5">
                    {identity?.isAuthenticated ?
                        <Link to="/packages" className="block mt-4 lg:inline-block lg:mt-0 text-teal-200 hover:text-white mr-4">Пратки</Link>
                        : null
                    }
                    {identity?.isAdmin ?
                        <Link to="/users" className="block mt-4 lg:inline-block lg:mt-0 text-teal-200 hover:text-white mr-4">Потребители</Link>
                        : <></>
                    }
                    {identity?.isAuthenticated ?
                        <a onClick={signOut} className="block mt-4 lg:inline-block lg:mt-0 text-teal-200 hover:text-white mr-4">Изход</a>
                        :
                        <>
            ...
            <div className="w-full flex-grow lg:flex lg:items-center lg:w-auto hidden">
                <div className="text-sm lg:flex-grow">
                    ...
                    {identity?.isAuthenticated ?
                        <a onClick={signOut} className="block mt-4 lg:inline-block lg:mt-0 text-teal-200 hover:text-white cursor-pointer mr-4">Изход</a>
                        :
                        <>
